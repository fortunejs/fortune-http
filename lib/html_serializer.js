  ...
  for (i = 0, j = fields.length; i < j; i++) {
    ...
    if (record) {
      ...
      if (Array.isArray(value) &&
        fieldType !== Object && fieldType.name !== 'Object') {
        value = value.slice()
    ...
    if ((fieldType === String || fieldType.name === 'String') &&
      !matchPassword)
      input.textArea = {
        name: field,
        value: value,
    ...
    else if (fieldType === Boolean || fieldType.name === 'Boolean')
      input.boolean = {
        name: field,
      ...
      inputType = 'text'
      if (fieldType === Date || fieldType.name === 'Date')
        inputType = 'datetime'
      if (fieldType === Number || fieldType.name === 'Number')
        inputType = 'number'
