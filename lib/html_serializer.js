...
var binding = {
  ...
  documentation: [ '.documentation', {
    title: '.title > span',
    titleLinks: [ '.record-type-link', {
      text: setText,
      href: setHref
    } ],
    descriptionTabChecked: [ '#description', setChecked ],
    mediaTypesTabChecked: [ '#media-types', setChecked ],
    indexDescriptionTitle: '.description-label',
    indexDescription: [ '.description', {
      text: setHtml,
      hasDescription: function (node, value) {
        node.classList[!value ? 'add' : 'remove']('empty')
      }
    } ],
    mediaTypesTitle: '.media-types-label',
...
module.exports = function (HttpSerializer) {
  ...
  function (contextResponse, request, response) {
    ...
    var indexRoute = this.options.indexRoute || ''
    ...
    // Show the index.
    else if (contextResponse.isTypeUnspecified || type === indexRoute) {
    ...
    for (k = 0, l = columns.length; k < l; k++) {
      ...
      if (field in record && value !== null) {
        ...
        case 'array':
          ...
          break
        case 'date':
          text = value.toJSON ? value.toJSON() : '' + value
          break
        case 'object':
        ...
        case 'boolean':
          ...
          break
        case 'buffer':
          count = value.byteLength || Buffer.byteLength(value)
          format = 'buffer'
...
function renderIndex (request) {
  ...
  var hasToken = 'token' in cookies
  var prefix = self.options.prefix || ''
  var indexRoute = self.options.indexRoute || ''
  var uriBase64 = self.options.uriBase64
  var settings = self.settings
  var message = self.message
  ...
  var definitions = (function () {
    ...
    Object.keys(recordTypes).forEach(function (type) {
      ...
      Object.getOwnPropertyNames(recordType).forEach(function (field) {
        ...
        var isEnumerable = recordType.propertyIsEnumerable(field)
        var tagType, tags
  ...
  var navigation = (function () {
    return Object.keys(recordTypes).map(function (type) {
      var recordType = recordTypes[type]
  ...
  var indexDescription = (function () {
    var result

    if (typeof settings.description === 'string')
      result = settings.description

    if (typeof settings.description === 'object')
      result = settings.description[language] ||
        settings.description[message.defaultLanguage]
  ...
  var data = {
    ...
    documentation: {
      ...
      descriptionTabChecked: Boolean(indexDescription),
      mediaTypesTabChecked: !Boolean(indexDescription),
      indexDescriptionTitle: message('Description', language),
      indexDescription: {
        text: indexDescription ||
          message('NoDescription', language),
        hasDescription: Boolean(indexDescription)
      },
...
function makeForm (request, type, record) {
  ...
  for (i = 0, j = fields.length; i < j; i++) {
    ...
    isEnumerable = recordType.propertyIsEnumerable(field)
    ...
    if (record) {
      ...
      if (Array.isArray(value) &&
        ((fieldType && fieldType !== Object && fieldType.name !== 'Object') ||
        definition[keys.link])) {
        value = value.slice()
    ...
    if (fieldType &&
      (fieldType === String || fieldType.name === 'String') &&
      !matchPassword)
      input.textArea = {
        name: field,
        value: value,
      ...
      (fieldType === Boolean || fieldType.name === 'Boolean'))
      input.boolean = {
        name: field,
      ...
      inputType = 'text'
      if (fieldType &&
        (fieldType === Date || fieldType.name === 'Date'))
        inputType = 'datetime'
      if (fieldType &&
        (fieldType === Number || fieldType.name === 'Number'))
        inputType = 'number'

      if (fieldType &&
        (fieldType === Buffer || fieldType.name === 'Buffer')) {
        inputType = 'file'
        value = null
      }
...
function setValue (node, value) { if (value !== null) node.value = value }
