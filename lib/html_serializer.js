...
module.exports = function (HttpSerializer) {
  var common, map, assign, filter, methods, message, errors, keys, encodeRoute
  ...
  function (contextResponse, request, response) {
    ...
    function renderIndex () {
      ...
      page.push(tag('div', {
        ...
        tag('div', { 'class': 'property-header' }, [
          tag('div', tag('h4', message('Class', language)))
        ]),
        tag('div', { 'class': 'property-list' }, renderClasses()),
        tag('div', { 'class': 'property-header' }, [
    ...
    function renderClasses () {
      return Object.keys(recordTypes).map(function (type) {
        return tag('div', {
          ...
          documentation.hasOwnProperty(type) ? tag('div', {
            'property': 'rdfs:comment'
          }, typeof documentation[type] === 'string' ?
            documentation[type] :
            typeof documentation[type] === 'object' ?
              documentation[type][language] ||
            documentation[type][message.defaultLanguage] :
              null) : null
        ]))
      })
    }
          ...
          tag('div', [
            ...
            fields[field].map(function (definition) {
              return tag('span', { 'class': 'type' },
                tag('span', definition[linkKey] || definition[typeKey].name))
            })
          ]),
    ...
    function renderInputGroup (type, key, record) {
      ...
      return [
        ...
        var isTextarea = fieldType === String
        return tag(isTextarea ? 'textarea' : 'input', assign({
          name: key,
          placeholder: placeholder(fieldType || fieldLink)
        }, !isTextarea ? { value: value } : null),
        isTextarea ? escapeHtml(value) : null)
      }))
    }
