 ...
 * hook for error logging. **IMPORTANT**: the returned Promise must be caught:
 *
 * ```js
 * listener(request, response)
 * .catch(error => {
 *   // Important: the returned Promise must be caught so that there are no
 ...
 *     useCompression: true,
 *
 *     // Use built-in ETag implementation, which uses CRC32 under the hood.
 *     // This option should be disabled in case of a reverse proxy which
 *     // handles compression.
 *     useETag: true
 *   }
 * }
  ...
  function end (contextResponse, request, response) {
    ...
    .then(function () {
      return new Promise(function (resolve) {
        ...
        if (useETag && payload) {
          etag = crc32.calculate(payload).toString(16)
