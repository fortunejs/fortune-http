...
var whitelistedHeaders = [
  'accept', 'accept-encoding', 'accept-language', 'authorization',
  'connection', 'content-length', 'content-type', 'date', 'host',
  'if-modified-since', 'if-none-match', 'if-range', 'origin', 'range',
  'referer', 'user-agent', 'upgrade'
]
...
function createListener (instance, options) {
  ...
  function listener (request, response) {
    ...
    if (serializerOutput === void 0) serializerOutput = negotiator.mediaType()
    ...
    return (~payloadMethods.indexOf(request.method) ?
      new Promise(function (resolve, reject) {
        ...
        request.on('data', function (chunk) { chunks.push(buffer(chunk)) })
      ...
      if (Buffer.isBuffer(payload) || typeof payload === 'string') {
        ...
        if (encoding && ~availableEncodings.indexOf(encoding))
          return zlib[encoding](payload, function (error, result) {
            ...
            meta.headers['content-length'] = String(payload.length)
            return resolve()
          })
